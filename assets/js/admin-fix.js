/**
 * Administración de UI Panel SaaS Menu Manager - Versión Corregida
 */
(function($) {
    'use strict';
    ...
    function initEvents() {
        ...
        $('#uipsm-add-item-form').on('submit', handleItemFormSubmit);
        
        // Cambio en el tipo de menú
        $('#uipsm-item-menu-type').on('change', toggleLinkOptions);
        
        // Vista previa del icono
        $('#uipsm-item-icon').on('input', updateIconPreview);
        
        // Selección de icono desde la biblioteca
        $('.uipsm-icon-item').on('click', selectIcon);
        
        // Botón de guardar menú completo
        $('#uipsm-save-menu').on('click', saveEntireMenu);
        
        // Botón de cancelar edición
        $('#uipsm-cancel-edit').on('click', cancelEdit);
        
        // Cambio en el parent_id (para prevenir ciclos)
        $('#uipsm-item-parent').on('change', validateParentSelection);
        
        // Usar delegación de eventos para los botones editar y eliminar
    ...
    function loadMenuItems() {
        $.ajax({
            ...
            error: function() {
                alert('Error de conexión al cargar los elementos del menú');
            }
        });
    }
    ...
    function renderMenuItemsRecursive(items, $container) {
        items.forEach(function(item) {
            ...
            $controls.append('<button type="button" class="uipsm-menu-edit" data-id="' + item.data.id + '"><span class="dashicons dashicons-edit"></span></button>');
            $controls.append('<button type="button" class="uipsm-menu-delete" data-id="' + item.data.id + '"><span class="dashicons dashicons-trash"></span></button>');
        ...
        // Ya no necesitamos eventos locales gracias a la delegación de eventos
    ...
    function handleItemFormSubmit(e) {
        ...
        $.ajax({
            ...
            error: function() {
                alert('Error de conexión al guardar el elemento');
            }
        });
    }
    ...
    function editMenuItem() {
        ...
        menuItems.forEach(function(existingItem) {
            if (parseInt(existingItem.id) === itemId) {
                item = existingItem;
            }
        });
